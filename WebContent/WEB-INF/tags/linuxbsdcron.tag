...
<c:choose>
	<c:when test="${not empty dynattrs.tab and dynattrs.tab == 'linux-bsd-cron'}">
	...
	<c:otherwise>
		<c:set var="divstyle" value="" />
					...
					<div class="tab-pane${divstyle}" id="linux-bsd-cron">
						<h2>linux bsd cron</h2>
...
curl_out=$( echo
url="https://www.duckdns.org/update?domains=${dynattrs.exampleSingleDomain}&token=${dynattrs.exampleToken}"
